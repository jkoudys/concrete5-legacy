...
if (is_object($layout) && is_object($a) && is_object($c)) {
    $db = Loader::db();
    $areaLayoutData = $db->getRow(
        'SELECT * FROM CollectionVersionAreaLayouts WHERE layoutID=? AND arHandle=? AND cID=? AND cvID=?',
        [$layoutID, $a->getAreaHandle(), intval($nvc->cID), intval($nvc->getVersionID())]
    );
    ...
    switch ($_GET['task']) {
        ...
        case 'delete':
            ...
            if ($areaNameNumber) {
                ...
                $db->Execute(
                    'delete from CollectionVersionAreaLayouts WHERE cID = ? AND cvID = ? AND arHandle = ? AND areaNameNumber = ?',
                    [$nvc->getCollectionID(), $nvc->getVersionID(), $a->getAreaHandle(), $areaNameNumber]
                );
                ...
                for ($i=1; $i<=$maxCell; $i++) {
                    if (intval($_REQUEST['deleteBlocks'])) {
                        $layout->deleteCellsBlocks($nvc, $i);
                    } else {
                        $layout->moveCellsBlocksToParent($nvc, $i);
                    }
                }
        ...
        case 'quicksave':
            ...
            if (count($layout->breakpoints) != ($layout->columns-1)) {
                ...
                $saved = $layout->save();
                if ($oldLayoutId && !$layoutPreset) {
                    $nvc->updateAreaLayoutId(intval($cvalID), $layout->layoutID);
                }
...
if (!$jsonData['msg'] && !intval($jsonData['success'])) {
    $jsonData['msg']=t('Unknown Error');
}
if (!$jsonData['msg'] && intval($jsonData['success'])) {
    $jsonData['msg']=t('Success');
}

if ($_GET['task']=='deleteOpts') {
    echo $jsonData['msg'];
} else {
