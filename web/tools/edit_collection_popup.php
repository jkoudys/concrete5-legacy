...
<?php if ($_REQUEST['toppane'] == 1) {
    Loader::element('pane_header', ['c' => $c]);
}
?>
...
<?php if (!$_GET['close']) {
    if (!$c->isEditMode() && (!in_array($_GET['ctask'], ['add', 'edit_external', 'delete_external']))) {
    ...
    if (($c->isEditMode() || (in_array($_GET['ctask'], ['add', 'edit_external', 'delete_external']))) && $toolSection) {
        $args = [ 
            'c' => $c,
            'cp' => $cp,
            'ct' => $ct,
...
<?php if ($_REQUEST['toppane'] == 1) { ?>
    <?php Loader::element('pane_footer', ['c' => $c]); ?>
<?php } ?>
