        ...
        <div class="ccm-pane-options-permanent-search">
            ...
            <div class="span6">
                ...
                <div class="controls">
                    <select
                        class="span3"
                        onchange="window.location.href='<?= Loader::helper('navigation')->getLinkToCollection($c) ?>?asGroupAttributes=' + this.value"
                        id="asGroupAttributes"
                        name="asGroupAttributes"
                    >
                        <option
                            value="1"
                            <?= ($_REQUEST['asGroupAttributes'] !== '0') ? 'selected' : '' ?>
                        >
                            <?= t('Grouped by set') ?>
                        </option>
                        <option
            ...
            <a
                ...
                id="ccm-list-view-customize-top"
            >
    ...
    if (count($sets) > 0 && ($_REQUEST['asGroupAttributes'] !== '0')) {
        ...
        <?php } else { ?>
            <div
                ...
                id="asID_<?= $as->getAttributeSetID() ?>"
            >
            <?php
            foreach ($setattribs as $ak) { ?>
                <div
                    class="ccm-attribute"
                    id="akID_<?= $as->getAttributeSetID() ?>_<?= $ak->getAttributeKeyID() ?>"
                >
                    <img
                        class="ccm-attribute-icon"
                        src="<?= $ak->getAttributeKeyIconSRC() ?>"
                        width="16"
                        height="16"
                    />
