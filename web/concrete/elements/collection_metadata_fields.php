...
<div id="ccm-attributes-column" class="span3">
    ...
    <?php foreach ($sets as $as) { ?>
        ...
        foreach ($setattribs as $ak) {
            ...
            <li id="sak<?=$ak->getAttributeKeyID()?>" class="ccm-attribute-key ccm-attribute-available <?php if (in_array($ak->getAttributeKeyID(), $usedKeysCombined)) {
?>ccm-attribute-added<?php
} ?>"><a style="background-image: url('<?=$ak->getAttributeKeyIconSRC()?>')" href="javascript:void(0)" onclick="ccmShowAttributeKey(<?=$ak->getAttributeKeyID()?>)"><?=$ak->getAttributeKeyDisplayName()?></a></li>
...
<div id="ccm-page-attributes-none" <?php if (count($usedKeysCombined) > 0) {
...
foreach ($attribs as $ak) {
    ...
    $caValue = $c->getAttributeValueObject($ak); ?>
        ...
        <div class="well" id="ak<?=$ak->getAttributeKeyID()?>" <?php if (!in_array($ak->getAttributeKeyID(), $usedKeysCombined)) {
?> style="display: none" <?php
} ?>>

        <?php if (in_array($ak->getAttributeKeyID(), $allowedAKIDs)) { ?>
        <input type="hidden" class="ccm-meta-field-selected" id="ccm-meta-field-selected<?=$ak->getAttributeKeyID()?>" name="selectedAKIDs[]" value="<?php if (!in_array($ak->getAttributeKeyID(), $usedKeysCombined)) {
?>0<?php
} else {
?><?=$ak->getAttributeKeyID()?><?php
} ?>" />
