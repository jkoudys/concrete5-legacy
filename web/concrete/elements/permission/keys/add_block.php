...
<div class="clearfix">
	...
	<ul class="inputs-list" <?php if ($assignment->getBlockTypesAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($blockTypes as $bt) { ?>
			<li><label><input type="checkbox" name="btIDInclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$bt->getBlockTypeID()?>" <?php if (in_array($bt->getBlockTypeID(), $assignment->getBlockTypesAllowedArray())) { ?> checked="checked" <?php } ?> /> <span><?=t($bt->getBlockTypeName())?></span></label></li>
		<?php } ?>
	</ul>
	</div>
</div>
...
<div class="clearfix">
	...
	<ul class="inputs-list" <?php if ($assignment->getBlockTypesAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($blockTypes as $bt) { ?>
			<li><label><input type="checkbox" name="btIDExclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$bt->getBlockTypeID()?>" <?php if (in_array($bt->getBlockTypeID(), $assignment->getBlockTypesAllowedArray())) { ?> checked="checked" <?php } ?> /> <span><?=t($bt->getBlockTypeName())?></span></label></li>
		<?php } ?>
	</ul>
	</div>
</div>
