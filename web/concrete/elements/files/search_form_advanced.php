		...
		<div class="span4" style="width: 280px">
			...
			<div class="input">
				<select multiple name="fsID[]" class="chosen-select">
					...
					<?php foreach($s1 as $s) { ?>
						<option value="<?=$s->getFileSetID()?>"  <?php if ((is_array($searchRequest['fsID']) && in_array($s->getFileSetID(), $searchRequest['fsID'])) || (is_string($searchRequest['fsID']) && $searchRequest['fsID'] == $s->getFileSetID())) { ?> selected="selected" <?php } ?>><?=wordwrap($s->getFileSetName(), '23', '&shy;', true)?></option>
					<?php } ?>
					</optgroup>
					<optgroup label="<?=t('Other')?>">
						<option value="-1" <?php if ((is_array($searchRequest['fsID']) && in_array(-1, $searchRequest['fsID'])) || (is_string($searchRequest['fsID']) && $searchRequest['fsID'] == '-1')) { ?> selected="selected" <?php } ?>><?=t('Files in no sets.')?></option>
					</optgroup>
				</select>
			</div>
