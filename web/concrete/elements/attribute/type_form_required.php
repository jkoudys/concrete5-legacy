...
<?php if (is_object($key)) { ?>
    ...
    <?= $ih->button_js(t('Delete Attribute'), "deleteAttribute()", 'right', 'error') ?>
<?php } ?>
...
<fieldset>
    ...
    switch ($category_handle) {
        ...
        <li>
            <label>
                <?= $form->checkbox('akIsSearchableIndexed', 1, $akIsSearchableIndexed) ?> <span><?= $keyword_label ?></span>
            </label>
        </li>
        <li>
            <label>
                <?= $form->checkbox('akIsSearchable', 1, $akIsSearchable) ?> <span><?= $advanced_label ?></span>
            </label>
        </li>
    </ul>
    </div>
