...
<?php if (!$b->getProxyBlock()) { ?>
    ...
    <a style="float:left" href="javascript:void(0)" <?php if ($replaceOnUnload) {
?>onclick="location.href='<?=DIR_REL?>/<?=DISPATCHER_FILENAME?>?cID=<?=$closeWindowCID ?><?=$step?>'; return true" class="btn"<?php
} else {
?>class="btn" onclick="ccm_blockWindowClose()" <?php
} ?>><?=t('Cancel')?></a>
    </div>
<?php } ?>
