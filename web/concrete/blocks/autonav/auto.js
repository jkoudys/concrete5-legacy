...
function autonavTabSetup() {
  $('ul#ccm-autonav-tabs li a').each((num, el) => {
    el.onclick = function () {
      const pane = this.id.replace('ccm-autonav-tab-', '');
      autonavShowPane(pane);
    };
  });
};

function autonavShowPane(pane) {
  $('ul#ccm-autonav-tabs li').each((num, el) => { $(el).removeClass('active'); });
  $(document.getElementById('ccm-autonav-tab-' + pane).parentNode).addClass('active');
  $('div.ccm-autonavPane').each((num, el) => { el.style.display = 'none'; });
