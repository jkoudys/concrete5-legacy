...
<select name="orderBy">
    <option value="display_asc" <?php if ($info['orderBy'] == 'display_asc') {
?> selected<?php
} ?>><?=t('in their sitemap order.')?></option>
    <option value="chrono_desc" <?php if ($info['orderBy'] == 'chrono_desc') {
?> selected<?php
} ?>><?=t('with the most recent first.')?></option>
    <option value="chrono_asc" <?php if ($info['orderBy'] == 'chrono_asc') {
?> selected<?php
} ?>><?=t('with the earliest first.')?></option>
    <option value="alpha_asc" <?php if ($info['orderBy'] == 'alpha_asc') {
?> selected<?php
} ?>><?=t('in alphabetical order.')?></option>
    <option value="alpha_desc" <?php if ($info['orderBy'] == 'alpha_desc') {
?> selected<?php
} ?>><?=t('in reverse alphabetical order.')?></option>
    <option value="display_desc" <?php if ($info['orderBy'] == 'display_desc') {
?> selected<?php
} ?>><?=t('in reverse sitemap order.')?></option>
</select>
<br><br>
...
?> selected<?php
} ?>><?=t('Relevant sub pages.')?></option>
    <option value="relevant_breadcrumb"<?php if ($info['displaySubPages'] == 'relevant_breadcrumb') {
?> selected<?php
} ?>><?=t('Display breadcrumb trail.')?></option>
...
<select id="displaySubPageLevels" name="displaySubPageLevels" <?php if ($info['displaySubPages'] == 'none') {
?> disabled <?php
...
?> selected<?php
    ...
    <option value="enough_plus1"<?php if ($info['displaySubPageLevels'] == 'enough_plus1') {
?> selected<?php
...
<div id="divSubPageLevelsNum"<?php if ($info['displaySubPageLevels'] != 'custom') {
?> style="display: none"<?php
} ?>>
    <br>
