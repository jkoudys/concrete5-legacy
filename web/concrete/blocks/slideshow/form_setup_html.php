...
<div id="newImg">
    ...
    <select name="type" style="vertical-align: middle">
        <option value="CUSTOM"<?php if ($type == 'CUSTOM') {
?> selected<?php
} ?>><?=t('Custom Slideshow')?></option>
        <option value="FILESET"<?php if ($type == 'FILESET') {
?> selected<?php
} ?>><?=t('Pictures from File Set')?></option>
    </select>
    </td>
    <td>
    <strong><?=t('Playback')?></strong>
    <select name="playback" style="vertical-align: middle">
        <option value="ORDER"<?php if ($playback == 'ORDER') {
?> selected<?php
} ?>><?=t('Display Order')?></option>
        <option value="RANDOM-SET"<?php if ($playback == 'RANDOM-SET') {
?> selected<?php
} ?>><?=t('Random (But keep sets together)')?></option>
        <option value="RANDOM"<?php if ($playback == 'RANDOM') {
?> selected<?php
} ?>><?=t('Completely Random')?></option>
    </select>
    </td>
    </tr>
