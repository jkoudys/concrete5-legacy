...
if ($akSelectAllowMultipleValues) { ?>
    ...
    <?php foreach ($options as $opt) { ?>
        <label class="checkbox"><input type="checkbox" name="<?=$this->field('atSelectOptionID')?>[]" value="<?=$opt->getSelectAttributeOptionID()?>" <?php if (in_array($opt->getSelectAttributeOptionID(), $selectedOptions)) {
?> checked <?php
