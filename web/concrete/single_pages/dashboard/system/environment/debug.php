...
<?php echo $this->controller->token->output('update_debug')?>
    ...
    <div class="controls">
    <label class="radio">
    <input type="radio" name="debug_level" value="<?php echo DEBUG_DISPLAY_PRODUCTION?>" <?php  if ($debug_level == DEBUG_DISPLAY_PRODUCTION) {
?> checked <?php
} ?> /> <span><?php echo t('Hide errors from site visitors.')?> </span>
    </label>

    <label class="radio">
    <input type="radio" name="debug_level" value="<?php echo DEBUG_DISPLAY_ERRORS?>" <?php  if ($debug_level == DEBUG_DISPLAY_ERRORS) {
?> checked <?php
