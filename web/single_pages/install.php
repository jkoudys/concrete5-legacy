...
$routines = array_map(
    function ($routine) {
        return [
            'text' => $routine->getText(),
            'method' => $routine->getMethod(),
            'edge' => $this->url('/install', 'run_routine', $installPackage, $routine->getMethod()),
            'progress' => $routine->getProgress(),
        ];
    },
    (array) $installRoutines
);
...
<script type="text/javascript">
    $(function () {
      const routines = <?= json_encode(array_map($installRoutines)) ?>;
      let step = 0;

      function installRoutine({ text, method, edge, progress }) {
        $('#install-progress-summary').html(text || '');
        fetch(edge, {
            credentials: 'include',
        })
        .then((res) => res.json())
        .then(({ error, message }) => {
          if (error) {
            ...
            $('#install-progress-errors').append(`<div class='alert-message error'>${message}</div>`);
            $('#install-progress-error-wrapper').fadeIn(300);
          } else {
            $('#install-progress-bar div.bar').css('width', `${progress}%`);
            if (step < routines.length) {
              step++;
              installRoutine(routines[step]);
            } else {
              $('#install-progress-wrapper').fadeOut(300, () => $('#success-message').fadeIn(300));
            }
          }
        })
        .catch(({ responseText }) => {
          $("#install-progress-wrapper").hide();
          $("#install-progress-errors").append(`<div class="alert-message error">${responseText}</div>`);
          $("#install-progress-error-wrapper").fadeIn(300);
        });
      }
      if (routines.length) installRoutine(routines[step]);
    });
        ...
        <div class="span5 offset1">
            ...
            <fieldset>
                ...
                <div class="clearfix">
                    ...
                    <div class="input">
                        <?=$form->email('uEmail', ['class' => 'xlarge'])?>
                    </div>
                </div>
                <div class="clearfix">
                    <label for="uPassword"><?=t('Password')?>:</label>
                    <div class="input">
                        <?=$form->password('uPassword', ['class' => 'xlarge'])?>
                    </div>
                </div>
                <div class="clearfix">
                    <label for="uPasswordConfirm"><?=t('Confirm Password')?>:</label>
                    <div class="input">
                        <?=$form->password('uPasswordConfirm', ['class' => 'xlarge'])?>
                    </div>
                </div>
            </fieldset>
        </div>
        <div class="span5">
            <fieldset>
                ...
                <div class="clearfix">
                    ...
                    <div class="input">
                        <?=$form->text('DB_SERVER', ['class' => 'xlarge'])?>
                    </div>
                </div>
                <div class="clearfix">
                    <label for="DB_USERNAME"><?=t('MySQL Username')?>:</label>
                    <div class="input">
                        <?=$form->text('DB_USERNAME', ['class' => 'xlarge'])?>
                    </div>
                </div>
                <div class="clearfix">
                    <label for="DB_PASSWORD"><?=t('MySQL Password')?>:</label>
                    <div class="input">
                        <?=$form->password('DB_PASSWORD', ['class' => 'xlarge'])?>
                    </div>
                </div>
                <div class="clearfix">
                    <label for="DB_DATABASE"><?=t('Database Name')?>:</label>
                    <div class="input">
                        <?=$form->text('DB_DATABASE', ['class' => 'xlarge'])?>
                    </div>
                </div>
            </fieldset>
        </div>
