...
<div id="ccm-pagelistPane-add" class="ccm-pagelistPane">
    ...
    <div class="ccm-block-field-group">
      ...
      <select name="ctID" id="selectCTID">
        ...
        <option value="<?php echo $ct->getCollectionTypeID()?>" <?php  if ($controller->ctID == $ct->getCollectionTypeID()) {
?> selected <?php
} ?>>
      ...
      <label class="checkbox">
          <input type="checkbox" name="displayFeaturedOnly" value="1" <?php  if ($controller->displayFeaturedOnly == 1) {
?> checked <?php
} ?> />
    ...
    <div class="ccm-block-field-group">
        ...
        <div>
            ...
            <option value="0" <?php  if ($controller->cParentID == 0) {
?> selected="selected"<?php
} ?>><?php echo t('everywhere')?></option>
            <option value="<?php echo $c->getCollectionParentID()?>" <?php  if ($controller->cParentID == $c->getCollectionParentID()) {
?> selected="selected"<?php
} ?>>
            <?php echo t('at the current level')?></option>
            <option value="<?php echo $c->getCollectionID()?>" <?php  if ($bCID == $controller->cParentID || $controller->cThis) {
?> selected="selected"<?php
} ?>>
            <?php echo t('beneath this page')?></option>
            <option value="OTHER" <?php  if ($isOtherPage) {
?> selected="selected"<?php
} ?>>
...
?> style="display: none" <?php
    ...
    <div class="ccm-block-field-group">
        ...
        <label class="radio inline">
            <input type="radio" name="flatDisplay" value="0" <?php  if (!$controller->flatDisplay) {
?> checked<?php
} ?> /><?php echo t('Hierarchy')?>&nbsp;
        </label>
        <label class="radio inline">
            <input type="radio" name="flatDisplay" value="1" <?php  if ($controller->flatDisplay) {
?> checked<?php
} ?> /><?php echo t('Flat')?>
        </label>
    </div>
    ...
    <div class="ccm-block-field-group">
        ...
        <label class="radio inline">
            <input type="radio" name="defaultNode" value="current_page" <?php  if ($controller->defaultNode!='current_month') {
?> checked<?php
} ?> /><?php echo t('Current Page')?>&nbsp;
        </label>
        <label class="radio inline">
            <input type="radio" name="defaultNode" value="current_month" <?php  if ($controller->defaultNode=='current_month') {
?> checked<?php
} ?> /><?php echo t('Current Month')?>
        </label>
    </div>
    ...
    <div class="ccm-block-field-group">
        ...
        <label class="radio inline">
            <input type="radio" name="showDescriptions" value="0" <?php  if (!$controller->showDescriptions) {
?> checked<?php
} ?> /><?php echo t('Titles')?>&nbsp;
        </label>
        <label class="radio inline">
            <input type="radio" name="showDescriptions" value="1" <?php  if ($controller->showDescriptions) {
?> checked<?php
} ?> /><?php echo t('Titles &amp; Descriptions')?>
        </label>
    </div>

    <?php  if ($controller->truncateTitleChars==0 && !$controller->truncateTitles) {
        $controller->truncateTitleChars=128;
} ?>
    <div class="ccm-block-field-group">
    ...
    <?php  if ($controller->truncateChars==0 && !$controller->truncateSummaries) {
        $controller->truncateChars=128;
} ?>
