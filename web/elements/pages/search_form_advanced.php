	...
	<div id="ccm-<?=$searchInstance?>-search-field-base-elements" style="display: none">
		...
		<span class="ccm-search-option"  search-field="permissions_inheritance">
			<select name="cInheritPermissionsFrom">
				<option value="PARENT"<?php if ($req['cInheritPermissionsFrom'] == 'PARENT') { ?> selected <?php } ?>><?=t('Parent Page')?></option>
				<option value="TEMPLATE" <?php if ($req['cInheritPermissionsFrom'] == 'TEMPLATE') { ?> selected <?php } ?>><?=t('Page Type')?></option>
				<option value="OVERRIDE"<?php if ($req['cInheritPermissionsFrom'] == 'OVERRIDE') { ?> selected <?php } ?>><?=t('Itself (Override)')?></option>
			</select>
		</span>
