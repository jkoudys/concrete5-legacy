...
$.fn.rating = function (userOptions = {}) {
  ...
  this.each(function () {
    ...
    const eid = (this.name || 'unnamed-rating').replace(/\[|\]+/g, '_');
    const context = $(this.form || document.body);
    const raters = context.data('rating') || { count: 0 };
    ...
    // insert rating star
    const star = $(`<div class="rating-star rater-${control.serial}"><a title="${this.title || this.value}">${this.value}</a></div>`);
    $(this).after(star);
    ...
    if (typeof control.split === 'number' && control.split > 0) {
      ...
      let spi = (control.count % control.split), spw = Math.floor(stw / control.split);
      star
...
Object.assign($.fn.rating, {
  fill() { // fill to the current mouse position.
    const control = this.data('rating'); if (!control) return this;
  ...
  draw() { // set value and stars to reflect current selection
    const control = this.data('rating'); if (!control) return this;
    // Clear all stars
    this.rating('drain');
    // Set control value
  ...
  readOnly(toggle, disable) { // make the control read-only (still submits value)
    const control = this.data('rating'); if (!control) return this;
    // setread-only status
  ...
  disable() { // make read-only and never submit value
    this.rating('readOnly', true, true);
  }, // $.fn.rating.disable

  enable() { // make read/write and submit value
