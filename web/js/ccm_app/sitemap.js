...
const { CCM_TOOLS_PATH, CCM_DISPATCHER_FILENAME } = window;

let tr_activeNode = false;
...
let tr_parseSubnodes = true;
let tr_reorderMode = false;
const	tr_moveCopyMode = false;
let SITEMAP_LAST_DIALOGUE_URL = '';
let ccm_sitemap_html = '';

function showPageMenu(obj, e) {
  ccm_hideMenus();
  e.stopPropagation();
  /* now, check to see if this menu has been made */
  let bobj = $(`#ccm-page-menu${obj.cID}`);
  ...
  if (!bobj.get(0)) {
    ...
    if (obj.isTrash) {
      html += '<li><a class="ccm-menu-icon ccm-icon-delete-menu" onclick="ccm_sitemapDeleteForever(' + obj.instance_id + ',' + obj.cID + ', true)">' + ccmi18n_sitemap.emptyTrash + '<\/a><\/li>';
    } else if (obj.inTrash) {
      html += '<li><a class="ccm-menu-icon ccm-icon-search-pages" onclick="ccm_previewInternalTheme(' + obj.cID + ', false, \'' + ccmi18n_sitemap.previewPage + '\')">' + ccmi18n_sitemap.previewPage + '<\/a><\/li>';
      html += '<li class=\"ccm-menu-separator\"><\/li>';
      html += '<li><a class="ccm-menu-icon ccm-icon-delete-menu" onclick="ccm_sitemapDeleteForever(' + obj.instance_id + ',' + obj.cID + ', false)">' + ccmi18n_sitemap.deletePageForever + '<\/a><\/li>';
    } else if (obj.cAlias == 'LINK' || obj.cAlias == 'POINTER') {
      html += '<li><a class="ccm-menu-icon ccm-icon-visit" id="menuVisit' + obj.cID + '" onclick="window.location.href=\'' + CCM_DISPATCHER_FILENAME + '?cID=' + obj.cID + '\'">' + ccmi18n_sitemap.visitExternalLink + '<\/a><\/li>';
      ...
      if (obj.canDelete) {
        ...
        <li>
          <a
            class="ccm-menu-icon ccm-icon-delete-menu"
            dialog-append-buttons="true"
            id="menuDelete${obj.cID}"
            dialog-width="360"
            dialog-height="150"
            dialog-modal="false"
            dialog-append-buttons="true"
            dialog-title="${ccmi18n_sitemap.deleteExternalLink}"
            href="${CCM_TOOLS_PATH}/edit_collection_popup.php?rel=SITEMAP&cID=${obj.cID}&display_mode=${obj.display_mode}&instance_id=${obj.instance_id}&select_mode=${obj.select_mode}&ctask=delete_external"
          >
            ${ccmi18n_sitemap.deleteExternalLink}
          </a>
        </li>`;
      }
    } else {
      ...
      if (obj.canEditSpeedSettings) {
        ...
        <li>
          <a
            class="ccm-menu-icon ccm-icon-speed-settings-menu"
            dialog-on-close="ccm_sitemapExitEditMode(${obj.cID})"
            dialog-width="550"
            dialog-height="280"
            dialog-append-buttons="true"
            dialog-modal="false"
            dialog-title="${ccmi18n_sitemap.speedSettingsTitle}"
            id="menuSpeedSettings${obj.cID}"
            href="${CCM_TOOLS_PATH}/edit_collection_popup.php?rel=SITEMAP&cID=${obj.cID}&ctask=edit_speed_settings"
        >
            ${ccmi18n_sitemap.speedSettings}
          </a>
        </li>`;
      }
      ...
      if (obj.cNumChildren > 0) {
        ...
        const searchURL = CCM_DISPATCHER_FILENAME + '/dashboard/sitemap/search/?selectedSearchField[]=parent&cParentAll=1&cParentIDSearchField=' + obj.cID;
...
function fixResortingDroppables(){
  ...
  for (let i=0; i<DZs.length; i++) {
    let nodeID = $(DZs[i]).attr('id').substr(7); 
...
function deleteBranchFade(nodeID) {
  // hides branch and its drop zone
  if (Krete.animEffects) {
    $('#tree-node' + nodeID).fadeOut(300, () => {
      $('#tree-node' + nodeID).remove();
    });
    $('#tree-dz' + nodeID).fadeOut(300, () => {
...
function deleteBranchDirect(nodeID) {
  // hides branch and its drop zone
  $('#tree-node' + nodeID).remove();
  $('#tree-dz' + nodeID).remove();
}
...
function showBranch(nodeID) {
  const orig = $('#tree-node' + nodeID);
  $('#tree-node' + nodeID).show();
  $('#tree-dz' + nodeID).show();
}

function rescanDisplayOrder(nodeID) {
  setLoading(nodeID);
  let queryString = '?foo=1';
  const nodes = $('#tree-root' + nodeID).children('li.tree-node');
  for (i = 0; i < nodes.length; i++) {
    if ($(nodes[i]).hasClass('ui-draggable-dragging')) continue;
    queryString += '&cID[]=' + $(nodes[i]).attr('id').substring(9);
  }
  $.getJSON(CCM_TOOLS_PATH + '/dashboard/sitemap_update.php', queryString, (resp) => {
...
function selectMoveCopyTarget(instanceID, display_mode, select_mode, destCID, origCID) {
  ...
  let dialog_title = ccmi18n_sitemap.moveCopyPage;
  let dialog_url = CCM_TOOLS_PATH + '/dashboard/sitemap_drag_request.php?instance_id=' + instanceID + '&display_mode=' + display_mode + '&select_mode=' + select_mode + '&origCID=' + origCID + '&destCID=' + destCID;
  let dialog_height = 350;
...
function selectLabel(e, node) {
  const cNumChildren = node.attr('tree-node-children');
  if (node.attr('sitemap-select-mode') == 'move_copy_delete' || tr_moveCopyMode == true) {
    const destCID = node.attr('id').substring(10);
    const origCID = node.attr('selected-page-id');
    ...
    let callback = node.parents('[sitemap-wrapper=1]').attr('sitemap-select-callback');
...
function ccmSitemapHighlightPageLabel(cID, name) {
  let sp = $(`#tree-label${cID} > span`);

  if (sp.length === 0) {
    sp = $(`tr.ccm-list-record[cID=${cID}]`);
    if (sp.length > 0) {
...
function activateLabels(instance_id, display_mode, select_mode) {
  const smwrapper = $('ul[sitemap-instance-id=' + instance_id + ']');
  ...
  smwrapper.find('ul[tree-root-state=closed]').each(function () {
    ...
    const nodeID = $(this).attr('tree-root-node-id');
  ...
  if (select_mode == 'select_page' || select_mode == 'move_copy_delete') {
    smwrapper.find('li.ccm-sitemap-explore-paging a').each(function () {
      $(this).click(function () {
        const treeRootNode = $(this).parentsUntil('ul').parent().parentsUntil('ul').parent().attr('tree-root-node-id');
        $.fn.dialog.showLoader();
        $.get($(this).attr('href'), (r) => {
  ...
  if (display_mode == 'full' && (!select_mode)) {
    ...
    smwrapper.find('div.tree-label').droppable({
      ...
      drop(e, ui) {
        ...
        const destCID = $(this).attr('id').substring(10);
        const origCID = $(orig).attr('id').substring(9);
        if (destCID == origCID) return false;
        const dialog_url = CCM_TOOLS_PATH + '/dashboard/sitemap_drag_request.php?instance_id=' + instance_id + '&origCID=' + origCID + '&destCID=' + destCID;
...
function ccm_triggerProgressiveOperation(url, params, dialogTitle, onComplete, onError) {
  $.fn.dialog.showLoader();
    ...
    success(r) {
      ...
      $('<div id="ccm-dialog-progress-bar" />').appendTo(document.body).html(r).jqdialog({
        ...
        open(e, ui) {
          ...
          const totalItems = $('#ccm-progressive-operation-progress-bar').attr('data-total-items');
          params.push({
            'name': 'process',
            'value': '1',
          });
...
function ccm_doProgressiveOperation(url, params, totalItems, onComplete, onError) {
  $.ajax({
    url,
    dataType: 'json',
    type: 'POST',
    ...
    success(r) {
      if (r.error) {
        ...
        const totalItemsLeft = r.totalItems;
        // update the percentage
        const pct = Math.round(((totalItems - totalItemsLeft) / totalItems) * 100);
...
function ccm_refreshCopyOperations() {
  const dialogTitle = ccmi18n_sitemap.copyProgressTitle;
...
function moveCopyAliasNode(reloadPage) {
  const origCID = $('#origCID').val();
  const destParentID = $('#destParentID').val();
  const destCID = $('#destCID').val();
  const ctask = $('input[name=ctask]:checked').val();
  const instance_id = $('input[name=instance_id]').val();
  const display_mode = $('input[name=display_mode]').val();
  const select_mode = $('input[name=select_mode]').val();
  const copyAll = $('input[name=copyAll]:checked').val();
  const saveOldPagePath = $('input[name=saveOldPagePath]:checked').val();
  // DO THE DEED
  ...
  if (copyAll == 1) {
    ...
    ccm_triggerProgressiveOperation(
      ...
      dialogTitle, () => {
        $('.ui-dialog-content').dialog('close');
        openSub(instance_id, destParentID, display_mode, select_mode, () => { openSub(instance_id, destCID, display_mode, select_mode); });
      }
    );
  } else {
    $.fn.dialog.showLoader();

    $.getJSON(CCM_TOOLS_PATH + '/dashboard/sitemap_drag_request.php', {
      origCID,
      destCID,
      ctask,
        ...
        if (reloadPage == true) {
          if (typeof (CCM_LAUNCHER_SITEMAP) != 'undefined') {
            ...
            if (CCM_LAUNCHER_SITEMAP == 'search') {
              ...
              $('#ccm-' + CCM_SEARCH_INSTANCE_ID + '-advanced-search').ajaxSubmit((resp) => {
...
function toggleSub(instanceID, nodeID, display_mode, select_mode) {
  ccm_hideMenus();
  const container = $('ul[tree-root-node-id=' + nodeID + '][sitemap-instance-id=' + instanceID + ']');
...
function ccm_sitemapDeleteForever(instance_id, nodeID, isTrash) {
  const dialogTitle = (isTrash) ? ccmi18n_sitemap.emptyTrash : ccmi18n_sitemap.deletePages;
  ccm_triggerProgressiveOperation(
    ...
    () => {
      if (isTrash) {
        ...
        const container = $('ul[tree-root-node-id=' + nodeID + ']').parent();
...
function setLoading(nodeID) {
  const listNode = $('#tree-node' + nodeID);
  listNode.removeClass('tree-node-' + listNode.attr('tree-node-type'));
  listNode.addClass('tree-node-loading');
}

function removeLoading(nodeID) {
  const listNode = $('#tree-node' + nodeID);
  listNode.removeClass('tree-node-loading');
  listNode.addClass('tree-node-' + listNode.attr('tree-node-type'));
}

function openSub(instanceID, nodeID, display_mode, select_mode, onComplete) {
  setLoading(nodeID);
  const container = $('#tree-root' + nodeID);
  cancelReorder();
  ccm_sitemap_html = '';
  $.get(CCM_TOOLS_PATH + '/dashboard/sitemap_data.php?instance_id=' + instanceID + '&node=' + nodeID + '&display_mode=' + display_mode + '&select_mode=' + select_mode + '&selectedPageID=' + container.attr('selected-page-id'), (resp) => {
...
function closeSub(instanceID, nodeID, display_mode, select_mode) {
  const container = $('ul[tree-root-node-id=' + nodeID + '][sitemap-instance-id=' + instanceID + ']');
  if (tr_doAnim) {
    setLoading(nodeID);
...
function ccmSitemapExploreNode(instance_id, display_mode, select_mode, cID, selectedPageID) {
  $.fn.dialog.showLoader();
  $.get(CCM_TOOLS_PATH + '/dashboard/sitemap_data.php', { 'instance_id': instance_id, 'display_mode': display_mode, 'select_mode': select_mode, 'node': cID, 'selectedPageID': selectedPageID }, (resp) => {
...
function ccmSitemapLoad(instance_id, display_mode, select_mode, node, selectedPageID, onComplete) {
  if (select_mode == 'move_copy_delete' || select_mode == 'select_page') {
    ...
    if (select_mode != 'move_copy_delete' && select_mode != 'select_page') {
      $('ul[sitemap-instance-id=' + instance_id + ']').sortable({
        ...
        opacity: 0.5,
        stop(sor) {
          const ss = $('ul[sitemap-instance-id=' + instance_id + ']').sortable('toArray');
          let queryString = '';
...
function ccm_sitemapSetupSearch(instance_id) {
  ccm_setupAdvancedSearch(instance_id);
  ccm_sitemapSetupSearchPages(instance_id);
  window.ccm_searchActivatePostFunction[instance_id] = function () {
    ccm_sitemapSetupSearchPages(instance_id);
...
function ccm_sitemapSearchSetupCheckboxes(instance_id) {
  ...
  $('td.ccm-' + instance_id + '-list-cb input[type=checkbox]').click((e) => {
    e.stopPropagation();
...
function ccm_sitemapSetupSearchPages(instance_id) {
  $(`#ccm-${instance_id}-list tr`).click(function (e) {
    ...
    if (node.attr('sitemap-select-mode') == 'select_page') {
      let callback = node.attr('sitemap-select-callback');
      if (callback == null || callback == '' || typeof (callback) == 'undefined') {
        callback = 'ccm_selectSitemapNode';
      }
...
function ccm_sitemapSelectDisplayMode(instance_id, display_mode, select_mode, selectedPageID) {
  // finds the selector for the instance of the sitemap and reloads it to be this mode

  const ul = $(`ul[sitemap-instance-id=${instance_id}]`)
  .html('')
  .attr('sitemap-display-mode', display_mode)
  .attr('sitemap-select-mode', select_mode)
  .attr('sitemap-display-mode', display_mode);

  const node = (display_mode === 'explore') ? 1 : 0;

  ccmSitemapLoad(instance_id, display_mode, select_mode, node, selectedPageID, () => {
...
function ccm_sitemapDeletePages(searchInstance) {
  const params = $('#ccm-' + searchInstance + '-delete-form').formToArray(true);
  ccm_triggerProgressiveOperation(
...
function ccm_sitemapUpdateDesign(searchInstance) {
  $('#ccm-' + searchInstance + '-design-form').ajaxSubmit((resp) => {
    ccm_parseJSON(resp, () => {
      $.fn.dialog.closeTop();
      ccm_deactivateSearchResults(searchInstance);
      $('#ccm-' + searchInstance + '-advanced-search').ajaxSubmit((resp) => {
...
$(() => {
  ...
  $('#ccm-show-all-pages-cb').click(function () {
    const showSystemPages = $(this).get(0).checked == true ? 1 : 0;
    $.get(CCM_TOOLS_PATH + '/dashboard/sitemap_data.php?show_system=' + showSystemPages, (resp) => {
      location.reload();
    });
  });
});
...
const Sitemap = {
  ...
  addResortDroppable,
  cancelReorder,
  ccmSitemapExploreNode,
  ccmSitemapHighlightPageLabel,
  ccmSitemapLoad,
  ccm_doProgressiveOperation,
  ccm_refreshCopyOperations,
  ccm_sitemapDeleteForever,
  ccm_sitemapDeletePages,
  ccm_sitemapExitEditMode,
  ccm_sitemapSearchSetupCheckboxes,
  ccm_sitemapSelectDisplayMode,
  ccm_sitemapSetupSearch,
  ccm_sitemapSetupSearchPages,
  ccm_sitemapUpdateDesign,
  ccm_triggerProgressiveOperation,
  closeSub,
  deleteBranchDirect,
  deleteBranchFade,
