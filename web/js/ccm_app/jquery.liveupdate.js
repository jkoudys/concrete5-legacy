...
(function ($) {
  ...
  $.liveUpdate.prototype = {
    init() {
      const self = this;
      this.setupCache();
      this.field.parents('form').submit(() => { return false; });
      this.field.keyup(() => { self.filter(); });
      self.filter();
    },
    ...
    displayResults(scores) {
      ...
      if (this.lutype == 'blocktypes') {
        ...
        this.list.children('li').removeClass('ccm-block-type-selected');
        $.each(scores, (i, score) => { self.rows[score[1]].addClass('ccm-block-type-available'); });
        ...
        $.each(scores, (i, score) => { self.rows[score[1]].addClass('ccm-attribute-available'); });
        ...
        this.list.children('li').removeClass('ccm-stack-selected');
        this.list.children('li').removeClass('ccm-item-selected');
        $.each(scores, (i, score) => { self.rows[score[1]].addClass('ccm-stack-available'); });
        ...
        $.each(scores, (i, score) => { self.rows[score[1]].show(); });
      }
    },

    getScores(term) {
      const scores = [];
      for (let i = 0; i < this.cache_length; i++) {
        const score = this.cache[i].score(term);
        if (score > 0) { scores.push([score, i]); }
      }
      return scores.sort((a, b) => { return b[0] - a[0]; });
    },
  };
})(jQuery);
